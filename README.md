...
Este repositorio se publica desde **khalilflores** (programación), con **Ciudadan** como organizador del proyecto.
...
## Para programadores que encuentren el repo
...
```bash
npm --prefix .\otromundial install
npm --prefix .\otromundial run dev
```
...
```bash
npm --prefix .\otromundial run build
npm --prefix .\otromundial run preview
```
